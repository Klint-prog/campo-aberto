...
class CreateFinancialExpenseFromAnimalPurchase
{
    ...
    {
        if ($event->event_name !== 'animal.purchased') {
            return null;
        }

        $payload = $event->payload ?? [];
        $amount = $payload['purchase_price'] ?? null;
        if ($amount === null) {
            return null;
        }

        return $this->financialTransactionService->createFromEvent(FinancialTransaction::TYPE_EXPENSE, 'Compra de animais', [
            'tenant_id' => $event->tenant_id,
            'farm_id' => $event->farm_id,
            'source_event_id' => $event->id,
            'description' => 'Despesa gerada por compra de animal',
            'amount' => $amount,
